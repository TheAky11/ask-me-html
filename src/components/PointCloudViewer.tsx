...
function PointCloud({ pointsCollected, progress }: { pointsCollected: number; progress: number }) {
...
function ScanningIndicator({ progress }: { progress: number }) {
...
export function PointCloudViewer({ isScanning, progress, pointsCollected }: PointCloudViewerProps) {
...
`;
  ...
  return (
    <Card className="border-scanner-primary/20 h-[600px] flex flex-col">
      ...
      <CardContent className="flex-1 p-0">
        <div className="w-full h-full bg-gradient-to-br from-scanner-console to-background rounded-b-lg relative overflow-hidden">
          ...
          >
            ...
            {pointsCollected > 0 && (
              <PointCloud pointsCollected={pointsCollected} progress={progress} />
            )}
          ...
          <div className="absolute top-4 left-4 space-y-2">
            ...
            {isScanning && (
              <Badge className="bg-scanner-primary/80 text-white border-none animate-pulse">
                Live Scanning
              </Badge>
            )}
          </div>

          {pointsCollected === 0 && !isScanning && (
